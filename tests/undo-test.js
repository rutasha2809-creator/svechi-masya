...
T.head('Возврат идёт в партию, из которой брали — в ту, что в работе');
/* Партию руками не выбирают: списание идёт по очереди, от самой старой,
   и отмена возвращает материал туда же. Старое поле lot в рецепте
   не должно менять ни списание, ни возврат. */
app.buyRows = [{ id: воск.id, q: '1', s: '1500', u: 'кг' }];
app.doBuy();
const партии = app.sortedLots(воск);
партии[1].d = партии[0].d + 86400000;
свеча.l[0].lot = партии[1].id;          // мусор от прежних версий
app.makeBatch(свеча, 2);
T.near(партии[0].q, 2000 - 200, 0.0001, 'списано из партии в работе, а не из «выбранной»');
T.near(партии[1].q, 1000, 0.0001, 'вторая партия не тронута');
app.logUndo(app.logIdOf(S.log.find(e => e.t === 'make')));
T.near(партии[0].q, 2000, 0.0001, 'вернулось в ту же первую партию');
T.near(партии[1].q, 1000, 0.0001, 'вторая так и осталась целой');
